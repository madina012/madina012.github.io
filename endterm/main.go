...
func (s *server) selectClubs() []ChildInfo {
...
func (s *server) selectClub(idC int) ChildInfo {
	rows := s.db.QueryRow("select club_id, club_name, price from clubs where club_id=?;", idC)
...
func (s *server) updateClubByID(w http.ResponseWriter, r *http.Request) {
	...
	idC := r.FormValue("idC")
	idInteger, err := strconv.Atoi(idC)
...
func updateClub(club_name string, price string, idC int, s *server) int {
	res, err := s.db.Exec("update clubs set club_name=?, price=?  where club_id=?", club_name, price, idC)
	if err != nil {
		log.Fatal(err)
	}
...
func deleteClub(idC int, s *server) {
	_, err := s.db.Exec("delete from clubs where club_id=?", idC)
	if err != nil {
		log.Fatal(err)
	}
}

func outputHTML2(w http.ResponseWriter, filename string, club ChildInfo) {
	t, err := template.ParseFiles(filename)
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	http.HandleFunc("/formClub", s.formClub)
